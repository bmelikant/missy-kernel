...
int create_page_directory() {
	...
	_page_directory[pdir_entry(0xffc00000)] = (uint32_t)(_page_directory) | PTABLE_FLAG_PRESENT;
	return 0;
}
...
int map_first_megabyte() {
	...
	_page_directory[pdir_entry(0)] = (uint32_t)(_first_pagetable) | PTABLE_FLAG_PRESENT;

	for (uint32_t i = 0; i < 0x100000; i += PAGE_SIZE) {
		_first_pagetable[ptable_entry(i)] = i | PTABLE_FLAG_PRESENT;
	}
	return 0;
}
...
int map_kernel() {
	...
	_page_directory[pdir_entry(0xc0100000)] = (uint32_t)(_kernel_pagetable) | PTABLE_FLAG_PRESENT;
	...
	for (uint32_t i = 0xc0100000; i < (uint32_t)(_kernel_end_virtual); i += PAGE_SIZE) {
		_kernel_pagetable[ptable_entry(i)] = (i - 0xc0000000) | PTABLE_FLAG_PRESENT;
	}
	return 0;
}
...
int create_new_kernel_stack() {
	...
	_page_directory[pdir_entry(KERNEL_STACK_MAPPING)] = (uint32_t)(_kernel_stack_map) | PTABLE_FLAG_PRESENT;
	_kernel_stack_map[ptable_entry(KERNEL_STACK_MAPPING)] = (uint32_t)(_kernel_stack_first_block) | PTABLE_FLAG_PRESENT;
	return 0;
}