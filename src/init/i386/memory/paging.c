...
int map_kernel() {
	...
	_page_directory[pdir_entry(0xc0000000)] = (uint32_t)(_kernel_pagetable) | PTABLE_FLAG_PRESENT;
	_kernel_end_virtual = kmemlow_get_kernel_endptr() + 0xc0000000 - 0x00100000;

	ki_printf("kernel end virtual = 0x%x\n", (uint32_t) _kernel_end_virtual);
	for (uint32_t i = 0xc0000000; i < (uint32_t)(_kernel_end_virtual); i += PAGE_SIZE) {
		_kernel_pagetable[ptable_entry(i)] = (0x100000 + (i - 0xc0000000)) | PTABLE_FLAG_PRESENT;
	}
	return 0;
}